...
/*Dau cham nhap nhay*/
    ...
    {   
        if(check_autohand==0)  
        {                  
            if(!SETUP && SETUP!=setup_old) //check the set key, NEU NHAN SET THÌ LAM
            {
                setting++;
                if(setting >= 8) setting=0;
            }   
            /*************************/        
            if(!SETDOWN && SETDOWN!=setdown_old) //check the set key, NEU NHAN SET THÌ LAM
            {
                setting--;
...
void main(void)
{
    ...
    dem=1;  // lênh nay cho phep chay chu
    while (1)
    {      
...
//                    buf_screen_1[k+16]=font_sohang1[giaima(day%10)+k];    
                    ...
                    buf_screen_1[k+24]=font_sohang2[giaima(0)+k];//cong them 80 sau k cu?i cùng neu nhu dat chung voi mang hang1
...
//                    buf_screen_1[k+32]=font_sohang1[giaima(month/10)+k];    
                 ...
                 for(k=0;k<8;k++)
                 {
                    ...
                    buf_screen_1[k+24]=text_dauxet[k];//cong them 80 sau k cu?i cùng neu nhu dat chung voi mang hang1
